...
const CreditNotesPage = () => {
    ...
    const loadNotes = async () => {
        try {
            const data = await purchaseService.getCreditNotes();
            setNotes(data);
        } catch (error) {
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800">Notas de Crédito (Proveedores)</h1>
                <button
                    onClick={() => navigate('/dashboard/purchases/credit-notes/new')}
                ...
                <div className="bg-white rounded-lg shadow overflow-x-auto">
                    <table className="min-w-[800px] sm:min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nro. NC</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {notes.length === 0 ? (
                                <tr><td colSpan="6" className="px-6 py-4 text-center text-gray-500">No hay notas de crédito registradas.</td></tr>
                            ) : notes.map(note => (
                                <tr key={note.id}>
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
                        <div className="flex justify-between items-center mb-4 pb-2 border-b">
                            <h2 className="text-xl font-bold text-gray-800">Detalles Nota de Crédito</h2>
