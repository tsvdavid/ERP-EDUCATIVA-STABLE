...
const TreasuryCreditNoteForm = () => {
    ...
    const loadInvoices = async () => {
        try {
            ...
            // Show only VALID/ISSUED invoices that can be credited
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await treasuryService.createCreditNote(formData);
            toast.success('Nota de Crédito generada exitosamente');
            navigate('/dashboard/treasury/credit-notes');
        } catch (error) {
            toast.error(error.response?.data?.error || 'Error al crear la nota de crédito');
        } finally {
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Nueva Nota de Crédito (Ventas)</h1>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end pt-4 space-x-3">
                        <button
                            type="button"
                            onClick={() => navigate('/dashboard/treasury/credit-notes')}
                        ...
                        >
                            {isSubmitting ? 'Guardando...' : 'Generar Nota de Crédito'}
                        </button>
                    </div>
                </form>
            </div>
...
export default TreasuryCreditNoteForm;
