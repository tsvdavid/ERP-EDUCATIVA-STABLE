...
const TreasuryDebitNotesPage = () => {
    ...
    const loadNotes = async () => {
        try {
            setIsLoading(true);
            const data = await treasuryService.getDebitNotes();
            setNotes(data);
        } catch (error) {
            toast.error('Error al cargar las notas de débito de ventas');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center text-left">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Notas de Débito (Ventas)</h1>
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Historial de notas de débito emitidas a clientes/estudiantes
                    </p>
                </div>
                <Link
                    to="/dashboard/treasury/debit-notes/new"
...
export default TreasuryDebitNotesPage;
